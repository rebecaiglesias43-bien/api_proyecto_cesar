from flask import current_app

class ProductoModel:
    def __init__(self, id_producto=None, nombre=None, precio=None, stock=None, estado=None):
        self.id_producto = id_producto
        self.nombre = nombre
        self.precio = precio
        self.stock = stock
        self.estado = estado
    
    def to_dict(self):
        return {
            'id_producto': self.id_producto,
            'nombre': self.nombre,
            'precio': float(self.precio) if self.precio else None,
            'stock': self.stock,
            'estado': self.estado
        }
    
    @staticmethod
    def listar_todos():
        cursor = current_app.mysql.connection.cursor()
        cursor.execute("SELECT pro_id, pro_nombre, pro_precio, pro_stock, pro_estado FROM productos")
        productos = cursor.fetchall()
        cursor.close()
        resultado = []
        for p in productos:
            resultado.append(ProductoModel(p[0], p[1], p[2], p[3], p[4]).to_dict())
        return resultado
    
    @staticmethod
    def obtener_por_id(id_producto):
        cursor = current_app.mysql.connection.cursor()
        cursor.execute("SELECT pro_id, pro_nombre, pro_precio, pro_stock, pro_estado FROM productos WHERE pro_id = %s", (id_producto,))
        p = cursor.fetchone()
        cursor.close()
        if p:
            return ProductoModel(p[0], p[1], p[2], p[3], p[4]).to_dict()
        return None
    
    @staticmethod
    def crear(nombre, precio, stock, estado):
        cursor = current_app.mysql.connection.cursor()
        cursor.execute("INSERT INTO productos (pro_nombre, pro_precio, pro_stock, pro_estado) VALUES (%s, %s, %s, %s)", (nombre, precio, stock, estado))
        current_app.mysql.connection.commit()
        id_generado = cursor.lastrowid
        cursor.close()
        return ProductoModel.obtener_por_id(id_generado)