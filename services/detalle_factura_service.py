...
class DetalleFacturaService:
    ...
    def listar_todos(self):
        ...
        cursor.execute("SELECT id_detalle, id_factura, id_servicio, subtotal FROM detalle_factura")
    ...
    def listar_por_factura(self, id_factura):
        cursor = self.mysql.connection.cursor()
        cursor.execute("SELECT id_detalle, id_factura, id_servicio, subtotal FROM detalle_factura WHERE id_factura = %s", (id_factura,))
        detalles = cursor.fetchall()
        cursor.close()
        resultado = []
        for d in detalles:
    ...
    def crear(self, id_factura, id_servicio, subtotal):
        ...
        # Obtener el recién creado
        cursor = self.mysql.connection.cursor()
        cursor.execute("SELECT id_detalle, id_factura, id_servicio, subtotal FROM detalle_factura WHERE id_detalle = %s", (id_generado,))
        d = cursor.fetchone()
        cursor.close()
        if d:
